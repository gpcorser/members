...
// NOTE: Email sending is centralized in members/includes/mailer.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'resend_verify') {
        if ((int)$user['is_verified'] === 1) {
            ...
            $sent = send_verification_email($user['email'], $verifyUrl);
        ...
        if (!$errors) {
            try {
                ...
                if ($willReverify) {
                    ...
                    $sent = send_verification_email($newEmail, $verifyUrl);
                    if ($sent) {
